  ...
  create or replace view `dwh-bootcamp-bigquery`.`latihan_dwh_bq_dbt_staging`.`stg_sales`
  OPTIONS()
...
with source as (
    select * from `dwh-bootcamp-bigquery`.`latihan_dwh_bq_dbt_seeds`.`raw_sales`
    -- PROD: select * from `dwh-bootcamp-bigquery`.`raw`.`raw_sales`
),
